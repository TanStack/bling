...
export default function mountRouter() {
  console.log('mounting islands router')

  const basePath = '/'
  let [currentLocation, setCurrentLocation] = createSignal<
  ...
  async function handleAnchorClick(evt: MouseEvent) {
    ...
    if (
      url.pathname === prevLocation.pathname &&
      url.search === prevLocation.search
    ) {
    ...
    if (await doNavigate(to, options)) {
  ...
  async function doNavigate(
    ...
    router.router.dispatchEvent(
      new CustomEvent('navigation-start', { detail: to }),
    )
    ...
    if (!response.ok) {
      ...
      router.router.dispatchEvent(
        new CustomEvent('navigation-error', {
          detail: { to, message: 'response not ok' },
        }),
      )
      return false
    }
    ...
    if (updated) {
      router.router.dispatchEvent(
        new CustomEvent('navigation-end', { detail: to }),
      )
      return true
    }
    ...
    router.router.dispatchEvent(
      new CustomEvent('navigation-error', {
        detail: { to, message: 'could not navigate' },
      }),
    )
    return false
  }
  ...
  router.router.addEventListener('navigation-start', console.log)
  router.router.addEventListener('navigation-end', console.log)
  router.router.addEventListener('navigation-error', console.log)

  window.router = router

  document.addEventListener('click', handleAnchorClick)
  window.addEventListener('popstate', handlePopState)
  console.log('mounted islands router')
}
