...
export function createRouteData<T, S>(
  ...
  const resourceFetcher: ResourceFetcher<S, T> = async (key: S) => {
    try {
      ...
      let response = await (fetcher as any).call({}, key)
  ...
  const [resource, { refetch }] = createResource<T, S>(
    ...
    dedupe(resourceFetcher),
    {
      storage: (init: T | undefined) =>
        createDeepSignal(init, options.reconcileOptions),
      ...options,
    } as any,
  )
...
function ensureQueryKeyArray<V extends any | any[], R = V extends [] ? V : [V]>(
  value: V,
): R {
